...
*/
var paramManager = require(__base + 'lib/utils/genesisParam');
...
exports.execute = function (request, response, runCtxt, callback) {
    ...
    async.waterfall([
       function (callback) {
				...
				if(bodyToParse){
						paramManager.parseXML(body,runCtxt,(err)=>{callback(err)});
				}
				callback(null);
			},
			function (callback) {
				...
				for (tpInd in operation.transferProperties) {
					...
					paramManager.extractParam(tp, body, headers, runCtxt , function (err,name,value) {
						...
						if(tpCount == operation.transferProperties.length){
						  paramManager.deleteParse(runCtxt,(err)=>{callback(err)});
						}
					});
				}
			},
