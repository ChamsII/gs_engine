			...
			function (callback) {
				async.eachSeries(operation.transferProperties,(tp,callback)=>{
					TpExtractor.extract(tp, request, runCtxt, (err,value)=>{
						...
						runCtxt.debug('STEP PARAM - Template ' + tp.template );
						if(tp.template !=undefined && tp.template != ""){
							if(tp.isUnique){
								if(value != "")
									runCtxt.parameters["template"] = tp.template;
							}else if( value == tp.value ){
								runCtxt.parameters["template"] = tp.template;
							}
						}

						if( tp.isFeeder !=undefined && tp.isFeeder ){
							runCtxt.parameters["feeder"] = {name: tp.feederName , value: value}
						}
