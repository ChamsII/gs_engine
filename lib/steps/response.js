    ...
    async.waterfall([
        	...
        	dataAccess.getTemplate(runCtxt,jdd.template, (err,tempFn) => {
                ...
                if( runCtxt.operation.feederProperties != undefined ) {
                    ...
                    async.eachSeries(feederProperties,(feed,callback)=> {
						...
						if( rowFeeder != undefined && rowFeeder.value != undefined ) {
							...
							async.eachSeries(feed.value,(fp,callback)=>{
								...
								if( fp.baliseResponse && fp.baliseResponse != undefined && randomeLine[fp.fileKey] && randomeLine[fp.fileKey] != undefined ){
							...
							runCtxt.debug('rowFeeder is undefined - STOP');
							return callback(null,jdd,start,resultText);
						}
		...
		function(jdd,start,resultText,callback){
			setTimeout(function(){
				...
				if( runCtxt.operation.fileResponse && runCtxt.operation.fileResponse != ""){
					
					var fileResponse = env.SIMUSPATH + "/" + runCtxt.basepath + "/" + runCtxt.api.name + "/" + runCtxt.operation.fileResponse
					runCtxt.debug( fileResponse )
					runCtxt.info('STEP RESPONSE FILE - ' + runCtxt.operation.fileResponse);
					fs.access(`./${fileResponse}`, (err)=>{
						if(err){
							...
							var readStream = fs.createReadStream(fileResponse);
