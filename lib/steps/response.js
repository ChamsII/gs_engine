...
var gs_db = (config.mode=='file')?require(__base + 'lib/dataAccess/fs/fs.js'):require(__base + 'lib/dataAccess/mongoAccess');
var fs = require('fs');
...
function execute(request, response, runCtxt, callback) {
    ...
    async.waterfall([
        function (callback) {
			...
			gs_db("dataset","get",args,{},null,(err,data) => {
        ...
        function(jdd,start,callback){
			var args={
				"service":runCtxt.service.basepath,
				"api":runCtxt.api.name,
				"template":jdd.template
			};
			gs_db("template","get",args,{},null,(err,data) => {
				if(err){
					return callback(err);
				}
				...
				return callback(null,jdd,start,tempFn,resultText);
			});
        },
        function(jdd,start,tempFn,msg,callback){
        ...
        function (jdd, start, callback) {
        	if(jdd.callback===undefined || jdd.callback.template===undefined){
				...
				var args={
					"service":runCtxt.service.basepath,
					"api":runCtxt.api.name,
					"template":jdd.callback.template
				};
				gs_db("template","get",args,{},null,(err,data) => {
					if(err){
						return callback(err);
