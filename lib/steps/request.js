...
function execute(request, response, runCtxt, callback) {
	...
	gs_db("service","get",{"service":basepath},{},null,(err,data) => {
		if(err){
		...
		//Check the API 
		for (var index in runCtxt.service.apis) {
			var api = runCtxt.service.apis[index];
		...
		var operation = api.operations[request.method];
