...
var GenesisParam = function GenesisParam(){
    var ParamSrc = { PATH: "PATH", QUERY: "QUERY", BODY_XPATH: "BODY_XPATH", HEADER: "HEADER" };
    ...
    this.extractParam = function (tp, body, headers, runCtxt , callback) {
        ...
        switch (tp.source) {
            ...
            case ParamSrc.BODY_XPATH:
                this.extractBODY_XPATH(tp.name, tp.path, tp.attr, body, runCtxt, callback);
                return;
    ...
    this.extractBODY_XPATH = function (name, searchPath, attr,body, runCtxt, callback) {
       
			if(searchPath.indexOf(' or ')!=-1){
				var searchPath1=searchPath.substring(0,searchPath.indexOf(' or '));
				var extractVal=runCtxt.bodyParsed.find(searchPath1); 
                if(extractVal!=undefined){
                    if(attr!=undefined){
                        extractVal=extractVal.get(attr);
                    }else{
    				...
    				var extractVal=runCtxt.bodyParsed.find(searchPath2);
                    if(extractVal!=undefined){
                        if(attr!=undefined){
                            extractVal=extractVal.get(attr);
                        }else{
                ...
                var extractVal=runCtxt.bodyParsed.find(searchPath);
                if(extractVal!=undefined){
                    if(attr!=undefined){
                        extractVal=extractVal.get(attr);
                    }else{
