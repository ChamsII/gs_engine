...
function execute(args,options,input,ctx,callback){
    if(ctx[CST.OBJ.DATASET]==undefined){
     ...
     fs.writeFile(`./${config.get('simusPath')}/${args[CST.OBJ.SERVICE]}/${args[CST.OBJ.API]}/${args[CST.OBJ.DATASET]}.json`, JSON.stringify(input) , (err)=>{
        if(err){
            return callback(new Error(util.format(CST.MSG_ERR.DATASET_UPDATE,args[CST.OBJ.DATASET])));
        }
