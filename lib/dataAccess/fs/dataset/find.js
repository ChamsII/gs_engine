...
function execute(args,options,input,ctx,callback){
    var jdd = `./${config.simusPath}/${args[CST.OBJ.SERVICE]}/${args[CST.OBJ.API]}/${args[CST.OBJ.DATASET]}.json`;

    fs.open(jdd, 'r', (err, fd) => {
        if (err){
            jdd = `./${config.simusPath}/${args[CST.OBJ.SERVICE]}/${args[CST.OBJ.API]}/default.json`;
            fs.open(jdd, 'r', (err, fd) => {
                if(err){
