...
function getServices(searchFilter,callback) {
	fsExt.subdirs(config.get('simusPath'), false, false, (err, subdirs) =>{
        if (err)
            return callback(err);
...
function execute(args,options,input,ctx,callback){
    ...
    if(args[CST.OBJ.SERVICE]==undefined){
        ...
        getConf(`${config.get('simusPath')}/${args[CST.OBJ.SERVICE]}`, (err, conf) => {
            if (err) {
                //logger.info("error getConf");
                return callback(err,args,options,input,ctx);
            }
