...
exports.loadService = function (basepath, cb) {
    getConf(`${config.simusPath}/${basepath}`, (err, conf) => {
        if (err) {
...
exports.changeServiceState = function(serviceName,state, callback){
	this.loadService(serviceName, (err,data)=> {
		...
		data.state = state;
		fs.writeFile(config.simusPath+'/'+data.basepath+'/'+data.basepath+'.json', JSON.stringify(data), function(err) {
			if(err) {
				callback(err);
			} else {
				callback(null,data);
...
exports.getTemplate = function (runCtxt,template, callback){
  var tempId=`./${config.simusPath}/${runCtxt.service.basepath}/${runCtxt.api.name}/${template}`;
...
exports.getDataSets= function (basepath, api, operation, pageNum, pageSize, searchFilter, callback) {
		var jddDir=`./${config.simusPath}/${basepath}/${api}`;
...
exports.getDataSet= function (basepath, api, operation, name, callback) {
		var jdd=`./${config.simusPath}/${basepath}/${api}/${name}.json`;
...
exports.getTemplateData= function (basepath, api, operation, name, callback) {
		var template=`./${config.simusPath}/${basepath}/${api}/${name}`;
...
exports.searchJDD = function(runCtxt,target,callback){
	var apiDir = `./${config.simusPath}/${runCtxt.service.basepath}/${runCtxt.api.name}`;
...
exports.deleteService=function(basepath ,callback){
	fs.access(`./${config.simusPath}/${basepath}`, (err)=>{
		if(err){
			callback(new Error('Le service n\'existe pas: '+basepath));
		}
		fs.remove(`./${config.simusPath}/${basepath}`, (err)=>{
			callback(err);
		});
	});
};
...
exports.renameService=function(old_val, new_val, callback){
	...
	async.waterfall([
		 function (callback) {
			 fs.access(`./${config.simusPath}/${new_val}`, (err) => {
				if(err){
					callback(null);
				}else{
		...
		function (callback) {
			fs.move(`./${config.simusPath}/${old_val}`,`./${config.simusPath}/${new_val}`, (err) => {
				if(err){
					fs.remove(`./${config.simusPath}/${new_val}`, (err) => {
		...
		function (callback) {
			fs.readFile(`./${config.simusPath}/${new_val}/${old_val}.json`, (err, data) => {
				if(err)
					return callback(err);
		...
		function (callback) {
			 fs.remove(`./${config.simusPath}/${new_val}/${old_val}.json`, (err)=>{
				 return callback(err);
			 });
		 }
		],function (err) {
...
exports.updateService=function(service,callback){
	fs.writeFile(`./${config.simusPath}/${service.basepath}/${service.basepath}.json`, JSON.stringify(service) , (err)=>{
		 callback(err);
	});
};
...
exports.deleteApi=function(basepath, api_name, callback){
	...
	async.waterfall([
		...
		function (service,callback) {
			fs.remove(`./${config.simusPath}/${basepath}/${api_name}`, (err)=>{
				 return callback(err,service);
			});
		}
	],function (err,data) {
...
exports.addApi=function(basepath, api_name, service, callback){
	...
	async.waterfall([
		...
		function (callback) {
			fs.mkdir(`./${config.simusPath}/${basepath}/${api_name}`, (err)=>{
				 callback(err);
			});
		},
		function (callback) {
			fs.writeFile(`./${config.simusPath}/${basepath}/${api_name}/${api_name}-.json`, `{\"template\":\"${api_name}--Template.xml\",\"delay\":0}` , (err)=>{
				callback(err);
			});
		},
		function (callback) {
			fs.writeFile(`./${config.simusPath}/${basepath}/${api_name}/${api_name}--Template.xml`, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><genesis>This is a simulator</genesis>" , (err)=>{
				callback(err);
			});
		},
		function (callback) {
	...
	async.waterfall([
		...
		function (service,callback) {
			...
			if(!apiNames.includes(new_api_data.name) 
				...
				fs.move(`./${config.simusPath}/${basepath}/${old_api_name}`,`./${config.simusPath}/${basepath}/${new_api_data.name}`, (err) => {
					return callback(err,service);
				});
			}else{
				return callback(null,service);
		...
		function (service,callback) {
			...
			fsExt.getFiles(`./${config.simusPath}/${basepath}/${new_api_data.name}`,false,false, (err,files) => {
				...
				toRename.forEach(function(oldfile,index,array){
					...
					fs.move(`./${config.simusPath}/${basepath}/${new_api_data.name}/${oldfile}`,`./${config.simusPath}/${basepath}/${new_api_data.name}/${newName}`,(err) => {
						return callback(err,service);
					});
				});
			});
...
exports.createService=function(service , callback){
	...
	async.series([
		 function (callback) {
			 fs.access(`./${config.simusPath}/${service.basepath}`, (err) => {
				if(err){
					callback(null);
				}else{
		 ...
		 function (callback) {
			 fs.mkdir(`./${config.simusPath}/${service.basepath}`, (err)=>{
				 callback(err);
			 });
		 },
		 function (callback) {
		 ...
		 function (callback) {
			  fs.mkdir(`./${config.simusPath}/${service.basepath}/${service.apis[0].name}`, (err)=>{
				 callback(err);
			 });
		  },
		  function (callback) {
			  fs.writeFile(`./${config.simusPath}/${service.basepath}/${service.apis[0].name}/${service.apis[0].name}-.json`, `{\"template\":\"${service.apis[0].name}--Template.xml\",\"delay\":0}` , (err)=>{
				 callback(err);
			 });
		  },
		  function (callback) {
			  fs.writeFile(`./${config.simusPath}/${service.basepath}/${service.apis[0].name}/${service.apis[0].name}--Template.xml`, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><genesis>This is a simulator</genesis>" , (err)=>{
				 callback(err);
			 });
		  }
	], function (err) {
