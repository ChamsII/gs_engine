...
exports.getDataSets= function (basepath, api, operation, pageNum, pageSize, callback) {
		var jddDir=`./${env.SIMUSPATH}/${basepath}/${api}`;
		
		fs.readdir(jddDir, function (err, files) {
				var tempArray = files.filter(function(file){return path.extname(file)=='.json';}).sort();
				var datasets = [];
