	...
	async.waterfall([
		...
		function (service, callback) {
			...
			if(apiNames.includes(new_api_data.name) 
				...
				return callback(new Error(`The Api ${new_api_data.name} already exists.`));
			}
		...
		function (callback) {
			//Si le nom d'API n'a pas ï¿½tï¿½ changï¿½ pas besoin de dï¿½placer les JDD
			if(new_api_data.name==old_api_name){
				return callback(new Error("NO_RENAME"));
			}
