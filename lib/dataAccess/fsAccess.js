...
/**
  * R�cup�re le fichier de configuration d'un simulateur
  *
  * @method getConf
...
exports.addDataset=function(basepath, api_name, key_name, callback){
	logger.debug(`[DATA] - AddDataset - Service [${basepath}] - Api [${api_name}] - Add Dataset ${key_name}.`);
	var self=this;
	...
	async.waterfall([
		...
		function (service,callback) {
			fs.writeFile(`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${key_name}.json`, `{\"template\":\"${api_name}-${key_name}-Template.xml\",\"delay\":0}` , (err)=>{
				callback(err, service);
			});
		},
		function (service, callback) {
			fs.writeFile(`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${key_name}-Template.xml`, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><genesis>This is a simulator</genesis>" , (err)=>{
				callback(err, service);
			});
		}
	],function (err,data) {
...
exports.updateDataset=function(basepath, api_name, key_name, dataset, callback){
	logger.debug(`[DATA] - updateDataset - Service [${basepath}] - Api [${api_name}] - Update Dataset ${key_name}.`);
	var self=this;
	...
	async.waterfall([
		...
		function (service,callback) {
			fs.move(`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${key_name}.json`,`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${dataset.name}.json`,(err) => {
				callback(err,service);
			});
		},
		function (service,callback) {
			fs.readFile(`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${dataset.name}.json`, { encoding: 'utf-8' }, (err, data) => {
					callback(err, service, JSON.parse(data));
			});
		},
		function (service, data, callback) {
			fs.writeFile(`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${dataset.name}.json`, `{\"template\":\"${api_name}-${dataset.name}-Template.xml\",\"delay\":0}` , (err)=>{
				callback(err, service, data);
			});
		},
		function (service, data, callback) {
			fs.move(`./${env.SIMUSPATH}/${basepath}/${api_name}/${data.template}`,`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${dataset.name}-Template.xml`,(err) => {
				callback(err,service);
			});
		}
	],function (err,data) {
...
exports.deleteDataset=function(basepath, api_name, key_name, callback){
	logger.debug(`[DATA] - deleteDataset - Service [${basepath}] - Api [${api_name}] - Delete Dataset ${key_name}.`);
	var self=this;
	...
	async.waterfall([
		...
		function (service,callback) {
			fs.remove(`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${key_name}.json`,(err) => {
				callback(err,service);
			});
		},
		function (service, callback) {
			fs.remove(`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${key_name}-Template.xml`,(err) => {
				callback(err,service);
			});
		}
	],function (err,data) {
...
exports.updateDetail=function(basepath, api_name, key_name, detail, callback){
	logger.debug(`[DATA] - updateDetail - Service [${basepath}] - Api [${api_name}] - Update Detail ${key_name}.`);
	var self=this;
	...
	async.waterfall([
		...
		function (service,callback) {
			fs.readFile(`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${key_name}.json`, { encoding: 'utf-8' }, (err, data) => {
					callback(err, service, JSON.parse(data));
			});
		},
		function (service, data, callback) {
			fs.writeFile(`./${env.SIMUSPATH}/${basepath}/${api_name}/${api_name}-${key_name}.json`, `{\"template\":\"${data.template}\",\"delay\":${detail.delay}}` , (err)=>{
				callback(err, service, data);
			});
		}
	],function (err,data) {
...
exports.getFileResponse = function(file, callback) {
	...
	async.waterfall([
		...
		function (service,callback) {
			if( operation.fileResponse != ""){
				var apiDir = `./${env.SIMUSPATH}/${basepath}/${api_name}`;
				var ope = `/${operation.fileResponse}`;
				fs.access(apiDir + ope , (err) => {
					if(err){
						fs.writeFile(`./${env.SIMUSPATH}/${basepath}/${api_name}/${operation.fileResponse}`, "" , (err)=>{
							return callback(err,service);
						});
					}else{
						return callback(null,service);
...
exports.updateApi=function(basepath, old_api_name, new_api_data, callback){
	...
	async.waterfall([
		...
		function (service,callback) {
			...
			//v�rifiation nom d'API unique
			if(!apiNames.includes(new_api_data.name) 
				&& old_api_name != new_api_data.name){
				//Renommer le r�pertoire de l'API dans le simulateur
		...
		function (service, callback) {
			//Mise � jour des donn�es de l'API
...
exports.createService=function(service , callback){
	...
	async.series([
		 ...
		 function (callback) {
		  ...
		  function (callback) {
			var operation = service.apis[0].operations[0];
			if( operation.fileResponse != ""){
				fs.writeFile(`./${env.SIMUSPATH}/${service.basepath}/${service.apis[0].name}/${operation.fileResponse}`, "" , (err)=>{
					callback(err);
				});
			}else{
				callback(null);
...
exports.changeServiceEncode = function(serviceName, api_name, responsetype, callback){
	this.loadService(serviceName, (err,data)=> {

		for(var id in data.apis){
			var api = data.apis[id];
			if(api.name == api_name){
				for(var idOpe in api.operations){

					data.apis[id].operations[idOpe].responseType = responsetype;
					fs.writeFile(env.SIMUSPATH+'/'+data.basepath+'/'+data.basepath+'.json', JSON.stringify(data), function(err) {
						if(err) {
							callback(err);
						} else {
...
exports.updateTemplate=function(basepath, template, apiname, callback){

	var self=this;
	async.waterfall([
		function (callback) {
			fs.writeFile(`./${env.SIMUSPATH}/${basepath}/${apiname}/${apiname}--Template.xml`,  `${template}`, (err)=>{
				callback(err);
			});
		},
		function (callback) {
			...
			if(template.substring(0,1) == '<'){
				//XML encoding
				self.changeServiceEncode(basepath, apiname, 'text/XML;charset=UTF-8',(err)=>{
					callback(err);
				});
			}else{
				//Json encoding
				self.changeServiceEncode(basepath, apiname, 'text/JSON;charset=UTF-8',(err)=>{
					callback(err);
				});
			}
		}
...
exports.editTemplate=function(basepath, api_name, key_name, template, callback){
	logger.debug(`[DATA] - editTemplate - Service [${basepath}] - Api [${api_name}] - Update Template ${key_name}.`);
	var self=this;
	...
	async.waterfall([
		...
		function (service, data, callback) {
			fs.writeFile(`./${env.SIMUSPATH}/${basepath}/${api_name}/${data.template}`, `${template.detail}` , (err)=>{
				callback(err, service, data);
			});
		}
	],function (err,data) {
...
exports.updateDelay=function(basepath, api_name, delay, callback){

	this.loadService(basepath, (err,data)=> {
		...
		for(var id in data.apis){
			...
			if(api.name == api_name){
				for(var idOpe in api.operations){
					...
					data.apis[id].operations[idOpe].delay = delay;
					fs.writeFile(env.SIMUSPATH+'/'+data.basepath+'/'+data.basepath+'.json', JSON.stringify(data), function(err) {
						if(err) {
							callback(err);
						} else {
