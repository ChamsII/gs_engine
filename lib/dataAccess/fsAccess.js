...
exports.renameService=function(basepath, service, callback){
	async.waterfall([
		function (callback) {
			fs.move(`./${env.SIMUSPATH}/${basepath}`,`./${env.SIMUSPATH}/${service.basepath}`, (err) => {
				if(err){
					fs.remove(`./${env.SIMUSPATH}/${service.basepath}`, (err) => {
						return callback(new Error(`Error while moving ${basepath} to ${service.basepath}.`));
					});
				}else{
		...
		function (callback) {
			fs.readFile(`./${env.SIMUSPATH}/${service.basepath}/${basepath}.json`, (err, data) => {
				if(err)
					return callback(err);
		...
		function (srv,callback) {
			 srv.basepath=service.basepath;
			 fs.writeFile(`./${env.SIMUSPATH}/${service.basepath}/${service.basepath}.json`, JSON.stringify(srv) , (err)=>{ 
				return callback(err);
			 });
		},
		function (callback) {
			 fs.remove(`./${env.SIMUSPATH}/${service.basepath}/${basepath}.json`, (err)=>{
				 return callback(err);
			 });
		 }
		],function (err) {
