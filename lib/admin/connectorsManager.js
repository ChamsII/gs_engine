...
var gsDb = (config.get('mode')=='file')?require(__base + 'lib/dataAccess/fs/fs.js'):require(__base + 'lib/dataAccess/fs/fs.js');
        ...
        if(srv.apis[idApi].operations[idOp].connectors!=undefined){
          ...
          for(var idConn in connectors){
            ...
            if(connector.type="MQ"){
              ...
              var command=config.get('connectors.MQ.cmd');
              var args=[];
              args.push("-Dlog4j.dir="+config.get('simusPath')+"/"+srv.basepath);
              args.push("-Dlog4j.lvl="+connector.loglvl);
              args.push("-Dlog4j.configurationFile="+config.get('connectors.MQ.logconf'));
              args.push("-cp");
              args.push(config.get('connectors.lib')+"/*");
              args.push(config.get('connectors.MQ.bin'));
              ...
              args.push("http://localhost:"+config.get('PORT')+"/"+srv.basepath+srv.apis[idApi].uri);
