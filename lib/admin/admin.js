...
updateDataSet = function(req, res, next) {
	logger.info(`ADMIN - [ADD-MODIFY] Dataset ${req.params.basepath} - ${req.params.api} - ${req.params.operation} -  ${req.params.name}`);

	dataAccess.updateDataSet(req.params.basepath, req.params.api,req.params.operation, req.params.name,req.body,(err,data) => {
		if(err){
			logger.error(`ADMIN - [ADD-MODIFY] Dataset ${req.params.basepath} ${req.params.api} ${req.params.operation} ${req.params.name} - Error : ${err.message}`);
			res.end(err.message);
		}
		else{
			res.header('Content-Type','application/json');
...
exports.createAdmin = function () {
	...
	server.put('/:basepath/:api/:operation/dataset/:name',updateDataSet);
	...
	server.put('/:basepath/:api/key/:key', addDataset);
	server.del('/:basepath/:api/key/:key', deleteDataset);
