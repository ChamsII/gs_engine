...
modifyApi = function(req, res, next) {
	logger.info(`ADMIN - [EDIT API] ${req.params.basepath} -  ${req.params.api}`);
	
	logger.debug(req.body);
	
	var api =  JSON.parse(req.body);

	dataAccess.updateApi(req.params.basepath,req.params.api,api,(err) => {
		if(err){
			logger.error(`ADMIN - [EDIT API] ${req.params.basepath} - ${req.params.api} - Error : ${err.message}`);
			res.writeHead(500);
			res.end(err.message);
		}
		else{
...
addApi = function(req, res, next) {
	...
	dataAccess.addApi(req.params.basepath,req.params.api,service,(err) => {
		if(err){
...
exports.createAdmin = function () {
	...
	server.post('/:basepath/:api', modifyApi);
	server.put('/:basepath/:api', addApi);
	server.del('/:basepath/:api', deleteApi);
	
	server.post('/:basepath/:api/:operation', addOperation);
